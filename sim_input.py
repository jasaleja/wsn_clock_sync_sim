        ...
        while line:
            for key, regex in regex_dictionary.items():
                ...
                if found:
                    match key:
                        case 'topology':
                            parsed_data.append(found.group(key))
                        case 'size':
                            dimensions = found.group(key).split('x')
                            parsed_data.append([int(dimensions[0]), int(dimensions[1])])
                        case 'node_amount':
                            parsed_data.append(int(found.group(key)))
                        case 'transmission_range':
                            parsed_data.append(int(found.group(key)))
                        case 'natural_skew':
                            parsed_data.append(float(found.group(key)))
                        case 'initial_offset':
                            parsed_data.append(int(found.group(key)))
                        case _:
                            pass
